...
class Trash extends Model{
    ...
    protected $table = CDatabase::trash;
  		...
  		try {
  			...
  			if($data != null){
  				...
  				if(is_array($trash_image_other) && !empty($trash_image_other)){
  					$path = FuncLib::getRootPath().'uploads/'.CGlobal::FOLDER_TRASH.'/'.$data->trash_folder.'/'.$data->trash_obj_id;
        ...
        if($id > 0){
            $_class =  "App\Modules\Models\\".$class;
  			if(class_exists($_class)){
  				...
  				if($result != null){
  					if($folder != ''){
  						$folder_trash = FuncLib::getRootPath().'uploads/'.CGlobal::FOLDER_TRASH.'/'.$folder.'/'.$id;
  						...
  						if($field_image_other != ''){
  							...
  							if($strImgOther != ''){
  								...
  								if(is_array($arrImgOther) && !empty($arrImgOther)){
  									foreach($arrImgOther as $img){
  										$file_current = FuncLib::getRootPath().'uploads/'.$folder.'/'.$id.'/'.$img;
  							...
  							$file_current = FuncLib::getRootPath().'uploads/'.$folder.'/'.$id.'/'.$result->$field_image;
        ...
        if(Session::token() === $token) {
            if ($id > 0) {
                ...
                if ($data != null) {
                    ...
                    $folder_current = FuncLib::getRootPath() . 'uploads/' . $data->trash_folder . '/' . $data->trash_obj_id;
                    ...
                    if (is_array($arrImgOther) && !empty($arrImgOther)) {
                        foreach ($arrImgOther as $img) {
                            $file_recyclebin = FuncLib::getRootPath() . 'uploads/' . CGlobal::FOLDER_TRASH . '/' . $data->trash_folder . '/' . $data->trash_obj_id . '/' . $img;
                    ...
                    if ($trash_image != '') {
                        $file_recyclebin = FuncLib::getRootPath() . 'uploads/' . CGlobal::FOLDER_TRASH . '/' . $data->trash_folder . '/' . $data->trash_obj_id . '/' . $trash_image;
